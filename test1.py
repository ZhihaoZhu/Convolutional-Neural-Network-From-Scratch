...
x = np.arange(1200).reshape(4,3,10,10)/1200
val_y = np.arange(4).reshape(4)
val_Y = np.zeros((val_y.shape[0], 10))
val_Y[np.arange(val_y.shape[0]), val_y] = 1

conv1 = convolution_forward(x, kernel1, padding=padding, conv_stride=1, MP_stride=2, layer=1)
...
delta2 = delta2.reshape(4,1,5,5)  # 可能需要transpose
